...
function moveBPFToPreviousStage(formContext) {
    ...
    // Use the movePrevious method
    formContext.data.process.movePrevious(function (status) {
        if (status == "success") {
            var quoteStageAttr = formContext.getAttribute("sb1_quotestage");
            if (quoteStageAttr) {
                var quoteStage = quoteStageAttr.getValue();

                if (quoteStage !== null && quoteStage > 0) {
                    quoteStageAttr.setValue(quoteStage - 1);
                }
            }
            console.log("Moved to the previous stage successfully.");
            // Add any additional logic here upon success
        }
    });
}
...
function markAsReject(primaryControl) {
    ...
    //callPowerAutomateForRejectAndChangeStage(recordId);      
