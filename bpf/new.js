...
SB1.Quote = (function () {
    ...
    async function callQuoteAPI(executionContext) {   // get details from eff date
    ...
    async function callSyncQuoteAPI(executionContext) {
        ...
        if (existingQuoteId) {
            // Already synced — do not call API again
        ...
        try {
            ...
            // below is the quote_id for crm
            var quoteFolderId = syncResponse?.Result?.Data?.EbsAdditionalDetail?.FolderVersion?.FolderVersionId;

            //var quoteFolderId = syncResponse?.Result?.Data?.FolderVersion?.FolderVersionId;
            ...
            // var customerField = formContext.getControl("customerid");
            //if (customerField) {
            //customerField.setDisabled(true);
            //}
