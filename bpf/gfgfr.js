...
function applyQuoteWorkflowLogic(formContext, config) {
    ...
    if (currentBpfStage === submitStage && !workflowPopupFired) {
        ...
        var apiResponse = submitQuoteToUW(formContext);
    ...
    // lastStageSentToFlow = workflowStageValue;
        ...
        callSyncApiOnChangeStage(folderVersionId, quoteType, workflowStageValue);
...
function callSyncApiOnChangeStage(folderVersionId,quoteType, workflowStageValue) {
    debugger;
...
function callRejectQuoteCustomApi(folderVersionId, recordId,formContext) {
    debugger;
